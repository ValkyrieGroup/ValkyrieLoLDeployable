    ...
    def update(self, ctx):
        ...
        if current_casting:
            ...
            if "attack" not in spell_name and spell_name not in not_attacks:
                ...
                Orbwalker.LastAttacked = self.auto_begin
        else:
...
class OrbwalkLastHit:
    ...
    def get_target(self, ctx, distance):
        lasthits = predict_minions_lasthit(ctx, ctx.minions.alive().enemy_to(ctx.player).on_screen().get(),
                                           ctx.minions.alive().ally_to(ctx.player).on_screen().get())
        if len(lasthits) == 0:
            return None
...
class OrbwalkLanePush:
    ...
    def get_target(self, ctx, distance):
        ...
        jungle_target = target_selector_monster.get_target(ctx, ctx.jungle.targetable().enemy_to(player).near(player,
                                                                                                              distance).get())
        if jungle_target:
        ...
        if len(sorted_minions) > 0:
            ...
            health = predict_minion_health(ctx, enemy_minion, best_target.attackers, t_until_player_hits, 0.0,
                                           MinionModifiers(ctx))
        ...
        possible_targets = ctx.turrets.targetable().enemy_to(player).near(player,
                                                                          distance).get() + ctx.others.targetable().enemy_to(
            player).near(player, distance).get()
        return target_selector.get_target(ctx, possible_targets)
...
def valkyrie_menu(ctx: Context):
    ...
    ui.help(
        'Delay percent account for lag. Usually 0.1 is a good value. You should experiment at high attack speeds with lethal tempo in a practice game.')
        ...
        if mis_obj.spell.src_index == player.index:
            ...
            if "attack" in mis_name and mis_name not in not_attacks:
                if ctx.time - mis_obj.first_seen > 0.15:
                    b_windup_time = 0
                    # ctx.warn("Resetting base off missile")
